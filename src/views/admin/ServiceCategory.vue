                ...
                <div class="p-8 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
                    ...
                    <div class="w-full mb-1">
                        <div class="flex items-center justify-between pb-4">
                            <h5 class="mb-2 text-3xl font-bold text-gray-900 dark:text-white">Service Categories</h5>
                        </div>
                        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                            ...
                            <!-- Modal toggle -->
                            <button 
                                @click="openServiceCategory" 
                                data-modal-target="serviceCategoryModal" 
                                class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" 
                                type="button"
                            >Create Service Category</button>

                            <!-- Main modal -->
                            <div id="serviceCategoryModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
                            <div class="relative w-full h-full max-w-md md:h-auto">
                                <!-- Modal content -->
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button" @click="closeModal" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="serviceCategoryModal">
                                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                                        <span class="sr-only">Close modal</span>
                                    </button>
                                    <div class="px-6 py-6 lg:px-8">
                                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Service Category</h3>
                                        <form class="space-y-6" action="#">
                                            <div>
                                                <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Service Category</label>
                                                <input 
                                                  ...
                                                  required
                                                >
                                                <p id="categoryCheck" class="mt-2 hidden text-xs text-red-600 dark:text-red-500">Service category can't be empty</p>
                                            </div>
                                            <button 
                                            type="button"
                                            ...
                                            >Submit Service Category</button>
                                            <button 
                                            type="button" 
                                            id="updateServiceCategory"
                                            class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                            @click="updateServiceCategory(activeServiceCategoryId)"
                                            >Update Service Category</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex flex-col mt-6">
                        <div class="overflow-x-auto rounded-lg">
                            <div class="inline-block min-w-full align-middle">
                                <div class="overflow-hidden shadow sm:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600" id="serviceCategoryTable">
                                        ...
                                        <tbody  class="">
                                            <tr 
                                                ...
                                                v-for="(serviceCategoryObj, index) in serviceCategoryObjs.data"
                                                :key="serviceCategoryObj.id"
                                            >
                                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                                    {{ index + 1 }}
                                                </td>
                                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                                    {{ serviceCategoryObj.category}}
                                                </td>
                                                <td class="p-4 space-x-2 whitespace-nowrap">
                                                    
                                                    <button 
                                                        type="button" 
                                                        @click="editServiceCategory(serviceCategoryObj.id)" 
                                                    ...
                                                    <button
                                                        ...
                                                        @click="deleteServiceCategory(serviceCategoryObj.id)" 
                                                    >
                        ...
                        <div class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div class="flex items-center mb-4 sm:mb-0">
                                ...
                                Showing&nbsp;<span class="font-semibold text-gray-900 dark:text-white">1-10&nbsp;</span>of&nbsp;<span class="font-semibold text-gray-900 dark:text-white">{{ serviceCategoryObjs.totalRecords }}</span>
                                </span>
                                </div>
                                ...
                                <li v-for="(item, index) in new Array(serviceCategoryObjs.pages)" :key="index">
                                    <button 
                                        type="button" 
...
export default {
  ...
  methods: {
    async tableData(pageNumber=1, search) {
      ...
      let filterRecords = db.serviceCategories.filter(function (value) {
    ...
    async navigateTable(pageNumber) {
      ...
      this.serviceCategoryObjs = await this.tableData(this.pageNumber, this.search);
    ...
    async getServiceCategory() {
        try {
            ...
            this.serviceCategoryObjs = await this.tableData();
            this.pages = this.serviceCategoryObjs.pages;
    ...
    async addServiceCategory() {
        if (this.category !== '') {
            document.getElementById('categoryCheck').classList.add('hidden');

            let serviceCategoryObj = { 
            ...
            document.getElementById('categoryCheck').classList.remove('hidden');
        }
    },

    async updateServiceCategory() {
      let serviceCategoryObj = { 
            id: this.activeServiceCategoryId,
            category: this.service_category,
            isUpdated: this.isNew ? 0 : 1,
        };
        
        await updateOne({
            url: `service-categories/${this.activeServiceCategoryId}`, 
            table: 'serviceCategories', 
            data: serviceCategoryObj, 
            tableId: this.activeServiceCategoryId, 
            isOffline: this.$store.state.isOffline
        })
        .then((res) => {
            if (res.status === 'success') {
    ...
    editServiceCategory(id) {
      document.getElementById('addServiceCategory').classList.add('hidden');
      document.getElementById('updateServiceCategory').classList.remove('hidden');

      this.category = '';

      let serviceCategoryObj = this.serviceCategoryObjs.data.find(i => id === i.id);

      this.activeServiceCategoryId = id
      this.category = serviceCategoryObj.category; 
      this.isNew = serviceCategoryObj.isNew;    
    ...
    async deleteServiceCategory(id) {
      try {
        this.$swal.fire({
            type: 'warning',
            title: "Are You Sure!",
            html: `Did you want to proceed to delete this service category?`,
    ...
    async syncData(connection) {
      if (!connection) {
        try {
          setTimeout(async () => {
            const isNewStatus = await postToServer('serviceCategories', 'service-categories/store', 'isNew', 1);
            const isUpdatedStatus = await postToServer('serviceCategories', 'service-categories/update', 'isUpdated', 1);
    ...
    async getDataFromServer(reloadTable=false) {
      ...
      // clear serviceCategories table (objectStore)
      db.serviceCategories.clear();

      await this.$store.dispatch('databaseTransaction', { table: 'serviceCategories', res: res.data.service_categories })
    ...
    openServiceCategory() {
      ...
      document.getElementById('updateServiceCategory').classList.add('hidden');
      document.getElementById('addServiceCategory').classList.remove('hidden');
      this.openModal();
    },

    openModal() {
      const $targetEl = document.getElementById('serviceCategoryModal');
  ...
  watch: {
    async search(searchInput) {
      this.serviceCategoryObjs = await this.tableData(1, searchInput);
      this.pages = this.serviceCategoryObjs.pages;
    }
  }
}
