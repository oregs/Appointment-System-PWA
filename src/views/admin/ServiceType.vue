...
export default {
  ...
  methods: {
    async tableData(pageNumber=1, search) {
      ...
      let filterRecords = db.serviceTypes.filter(function (value) {
        if (search !== undefined && search !== '') {
          return value.type.toLowerCase().includes(search.toLowerCase()) || value.service_category.category.toLowerCase().includes(search.toLowerCase());
        } else {
          return value;
        }
      });
    ...
    async prevRecord(pageNumber) {
      ...
      console.log(this.pageNumber);
      this.serviceTypeObjs = await this.tableData(this.pageNumber, this.search);
    ...
    setNavigationStatus(prevButton) {
      let currentButton = document.getElementById(`navigateButton-${this.pageNumber, this.search}`);
  ...
  watch: {
    async search(searchInput) {
      this.serviceTypeObjs = await this.tableData(1, searchInput);
      this.pages = this.serviceTypeObjs.pages;
    }
  },
}
